import type { DiagramButton, SoundVoicePreset, SoundWaveform, StradellaBassVoicing } from "./types";
...
export function notesForButton(button: DiagramButton, options?: Pick<SoundOptions, "stradellaBassVoicing">): SoundNote[] {
  ...
  if (button.kind === "bass-root" || button.kind === "bass-counterbass") {
    ...
    const voicing = options?.stradellaBassVoicing ?? "single-low";

    if (voicing === "single-middle") {
      return [{ pitchClass: button.pitchClass, octave: 3 }];
    }

    if (voicing === "low-and-middle") {
      return [
        { pitchClass: button.pitchClass, octave: 2 },
        { pitchClass: button.pitchClass, octave: 3 },
      ];
    }

    return [{ pitchClass: button.pitchClass, octave: 2 }];
  }
...
/** Plays a group of buttons together without arpeggiating them. */
